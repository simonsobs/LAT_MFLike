...
class CIBc_bat(PowerSpectrumFromFile):
	"""Power Spectrum for clustered CIB from template at low-ell to power law at high ell.
	   HTJ - after ACTPolFull FORTRAN"""
	
	def __init__(self):
		super().__init__(_get_power_file('cib_extra'))

class CIB_Planck_bat(Model):
    """Planck CIB template.
...
class tSZxCIB_Planck_bat(PowerSpectrumFromFile):
...
class tSZxCIB_bat(PowerSpectrumFromFile):
	"""Power Spectrum for tSZxCIB from template.
	   HTJ - after ACTPolFull FORTRAN"""
