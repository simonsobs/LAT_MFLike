...
class tSZxCIBCrossSpectrum(Model):
    r"""Cross-spectrum for the tSZxCIB term.
    
    Because the tSZxCIB term is non-multiplicative (i.e. it is not of the form
    f(nu1) * f(nu2)) we write out the frequency term explicitly. This means we do
    not take in an abstract SED class but instead compute the two terms from the
    SED directly and cross them.
    
    It is hacked together so that it looks similar to the other cross spectra from the frontend.
    ...
    def _get_repr(self):
    	cl_repr = self._cl._get_repr()
    	key = list(cl_repr.key())[0]
    	cl_repr[key + ' (Cl)'] = cl_repr.pop(key)
    	
    	return { type(self).__name__ : [cl_repr] }
    ...
    @staticmethod
    def mu(nu, nu_0, beta, temp):
    	x = 1e9 * constants.h * nu / (constants.k * temp)
    	x0 = 1e9 * constants.h * nu_0 / (constants.k * temp)
    	res = np.power(nu / nu_0, beta + 1.0) * (np.expm1(x0) / np.expm1(x))
    ...
    def eval(self, sed_kwargs = {}, cl_kwargs = {}):
        ...
        ----------
        sed_args : dict
            Arguments for which the `sed` is evaluated. Needs to contain at least the following keys:
            nu : float or array
                The frequencies at which the sed is evaluated.
            nu_0 : float
                The normalization frequency at which the sed is evaluated.
            beta : float
                The spectral index of the blackbody part.
            temp : float
        ...
        cross : ndarray
            Cross-spectrum. The shape is ``(..., freq, freq, ell)``.
        """
        if not 'nu' in sed_kwargs:
            raise KeyError('Frequency not given in SED arguments.')
        if not 'nu_0' in sed_kwargs:
            raise KeyError('Normalization not given in SED arguments.')
        if not 'beta' in sed_kwargs:
            raise KeyError('Spectral index not given in SED arguments.')
        if not 'temp' in sed_kwargs:
            raise KeyError('Temperature not given in SED arguments.')
        
        nu = sed_kwargs['nu']
        nu_0 = sed_kwargs['nu_0']
        beta = sed_kwargs['beta']
        temp = sed_kwargs['temp']
        
        nui = nu[..., np.newaxis]
        nuj = nui[..., np.newaxis]
        f_nui = tSZxCIBCrossSpectrum.f(nui)
        f_nuj = tSZxCIBCrossSpectrum.f(nuj)
        f_nu0 = tSZxCIBCrossSpectrum.f(nu_0)
        
        mu_i = tSZxCIBCrossSpectrum.mu(nui, nu_0, beta, temp)
        mu_j = tSZxCIBCrossSpectrum.mu(nuj, nu_0, beta, temp)
        
        return ((f_nui * mu_j + f_nuj * mu_i) / (2.0 * f_nu0)) * self._cl(**cl_kwargs)


class PlankCrossSpectrum(Model):
    r"""Planck multi-template Cross Spectrum.
    	Do not use, it's messy and cobbled together.
