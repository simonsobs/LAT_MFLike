...
class TheoryForge_MFLike:
    def __init__(
        ...
        if mflike is None:
            ...
            self.bandint_external_bandpass = mflike.band_integration[
                "external_bandpass"
            ]
    ...
    def get_modified_theory(self, Dls, **params):
                    ...
                    for s in self.requested_cls:
                        ...
                        for pa in self.polarized_arrays:
                            ...
                            if (o1 in order) and (o2 in order):
                                fg_dict[s,'all',f1,f2] += fg_dict[s,'all',o1,o2]/self.array_weights[f1]/self.array_weights[f2]
        ...
        for m in self.spec_meta:
            ...
            if p in ["tt", "ee", "bb"]:
                ...
                if self.defaults_cuts[
                    "symmetrize"
                ]:  # we average TE and ET (as we do for data)
                    dls_dict[p, m["nu1"], m["nu2"]] += cmbfg_dict[p, m["nu2"], m["nu1"]]
    ...
    def _get_foreground_model(self,ell = None,freqs_order=None,**fg_params):
        ...
        model["tt", "tSZ"] = fg_params["a_tSZ"] * self.tsz(
            {"nu": self.bandint_freqs, "nu_0": nu_0}, {"ell": ell, "ell_0": ell_0}
        )
    ...
    def _bandpass_construction(self, **params):
        ...
        if np.any(np.array(self.bandint_width) > 0):
            assert (
                self.bandint_nsteps > 1
            ), "bandint_width and bandint_nsteps not coherent"
            assert np.all(
            ...
            for ifr, fr in enumerate(self.freqs):
                ...
                nubtrue = np.linspace(
                    bandlow, bandhigh, self.bandint_nsteps, dtype=float
                )
                nub = np.linspace(
    ...
    def _get_rotated_spectra(self, dls_dict, **nuis_params):
        ...
        rot = syl.Rotation_alm(
            ell=self.l_bpws, spectra=dls_dict, cls=self.requested_cls
        )
    ...
    def _get_template_from_file(self, dls_dict, **nuis_params):
        ...
        templ_pars = {
            cls: np.zeros((len(self.freqs), len(self.freqs)))
